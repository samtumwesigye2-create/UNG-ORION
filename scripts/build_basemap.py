...
'<defs><linearGradient id="bg" x2="1" y2="1"><stop stop-color="#172734"/><stop offset="1" stop-color="#0e1d28"/></linearGradient><linearGradient id="land" x2="1" y2="1"><stop stop-color="#304f4b"/><stop offset=".48" stop-color="#294443"/><stop offset="1" stop-color="#243d40"/></linearGradient><linearGradient id="water" x2="1" y2="1"><stop stop-color="#143c56"/><stop offset="1" stop-color="#0b314b"/></linearGradient></defs>',
...
for country in land['countries']:
    ...
    s.append(f'<path d="{d}" fill="{ "url(#land)" if ug else "#21323a"}" stroke="{ "#9fc2b5" if ug else "#4b6470"}" stroke-width="{2.2 if ug else 1.1}"/>')
# Subtle elevation styling is illustrative; it does not claim measured relief.
s.append('<g clip-path="url(#uganda-clip)" fill="none" stroke="#6a9782" stroke-opacity=".11">')
for n in range(8):
    s.append(f'<ellipse cx="420" cy="{310+n*65}" rx="{340+n*25}" ry="{92+n*18}" stroke-width="2"/>')
s.append('</g>')
for lake in hydro['lakes']:
    s.append(f'<path d="{paths(lake["geometry"])}" fill="url(#water)" stroke="#4890ad" stroke-width="1.5"/>')
for river in hydro['rivers']:
    s.append(f'<path d="{paths(river["geometry"], close=False)}" fill="none" stroke="#54a3b5" stroke-opacity=".82" stroke-width="1.9"/>')
for name, lon, lat, size, color in [('UGANDA',32.75,1.55,20,'#add0c0'),('LAKE VICTORIA',32.76,-.90,13,'#76b8cf'),('LAKE ALBERT',31.12,1.6,9,'#7cb7c8'),('LAKE KYOGA',33.05,1.45,9,'#7cb7c8')]:
    x,y = project((lon,lat))
...
for name,lon,lat in [('KAMPALA',32.58,.35),('ENTEBBE',32.46,.04),('JINJA',33.2,.44),('GULU',32.3,2.77),('MBARARA',30.66,-.61),('MBALE',34.18,1.08),('FORT PORTAL',30.3,.67),('LIRA',32.9,2.25),('SOROTI',33.61,1.71),('MASAKA',31.74,-.33),('HOIMA',31.35,1.43)]:
    x,y=project((lon,lat))
    s.append(f'<circle cx="{x:.1f}" cy="{y:.1f}" r="4.2" fill="#9be4d3" fill-opacity=".23"/><circle cx="{x:.1f}" cy="{y:.1f}" r="2" fill="#9be4d3"/><text x="{x+8:.1f}" y="{y-7:.1f}" fill="#cee0d9" font-size="10" font-family="Arial,sans-serif" font-weight="bold" letter-spacing="1">{escape(name)}</text>')
s.append('</svg>')
